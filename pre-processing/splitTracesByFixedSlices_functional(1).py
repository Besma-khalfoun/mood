...
def splitTraceByFixedSlices(trace,sliceSize):
	#try:
	df = pd.read_csv(trace,names=['id','lat','lng','timestamp'])
	try:
		df['timestamp']=pd.to_datetime(df['timestamp'], unit='ms')
	except:
		df['timestamp']=pd.to_datetime(df['timestamp'])
...
def processCsvTraceFile(filename,inputDirectory,outputDirectory,sliceSize):
	...
	for key, value in outDfSet.items():
		...
		#print(value)
		value.to_csv(os.path.join(outputDirectory, name+"_"+str(key)+".csv"),index=False,columns=['lat','lng','timestamp'],header=False)
